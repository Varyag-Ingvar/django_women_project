...
class ContactForm(forms.Form):
    """класс формы обратной связи, наследуемся от стандартного Form"""
    name = forms.CharField(label='Имя', max_length=255)
    email = forms.EmailField(label='Email')
    content = forms.CharField(label='Форма', widget=forms.Textarea(attrs={'cols': 60, 'rows': 10}))
    captcha = CaptchaField(label='Введите код ')  # капча из установленного пакета django-simple-captcha
